...
export class EnterpriseRepositiryComponent implements OnInit {
  ...
  constructor(private auditMsgService: AuditMsgService, public dialog: MatDialog,  private logger: LogService) {
   }
  ...
  onSave(): void {
    if (this.isValidForm()){
      this.auditMsgService.saveEnterpriseRepoInfo(this.enterpriseRepoInfo).subscribe(data => {
        this.auditMsgService.getRepositoryConfigInfo().subscribe(details => {
  ...
  onEnterpriseRepo1TestMsg(): void {
    ...
    this.auditMsgService.sendTestMsg(testEnterpriseRepoInfo).subscribe(data => {
      ...
      console.log("err = "+err);
      this.logger.log(Level.SEVERE,"err = " + err);
  ...
  openDialog(data: string): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      this.logger.log(Level.INFO,"err = ");
